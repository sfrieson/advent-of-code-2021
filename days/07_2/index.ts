...
let _numberTriangle = new Map<number, number>([[0, 0], [1, 1]]);
function numberTriangle(num: number): number {
  if (!_numberTriangle.has(num)) {
    _numberTriangle.set(num, num + numberTriangle(num - 1));
  }

  return _numberTriangle.get(num)!;
}

function getFuelCost(position: number) {
  let effort = 0;
  stats.counts.forEach((count, pos) => {
    effort += numberTriangle(Math.abs(position - pos)) * count
  });
